...
const OrderDetSec = ({id, name, img, stars, price, offer}: Iproduct) => {
    ...
    <div className='pt-1 w-full flex flex-col'>
        ...
        <div className="grid md:grid-cols-2 gap-16 my-3">
            <ul className='p-2 flex flex-col place-items-center'>
                <Image width={1000} height={1000} src={img} className='w-[30%]' alt="Santosh" />
            ...
            <ul className='pl-[25%] md:pl-1'>
               ...
               [1, 2, 3, 4, 5].map((i) => (
                 ...
                 <p className='text-gray-400 text-xl'>({id} Review)</p></div>
                 <div className="flex flex-row gap-1 font-semibold"><del className='text-gray-400'>₹{price}</del><span>₹{disPrice}</span><span className='text-green-600'>{offer}% OFF</span></div>
