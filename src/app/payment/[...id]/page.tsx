...
const page = () => {
  ...
  // ------------------------------------------
  const price = (productData?.price / 100) * productData?.offer;
  const discount = Math.round(price)
  const prices = productData?.price - discount;
          ...
          <div className="p-4">
            ...
            <div className='p-2 py-8'>
                <ul className='grid md:grid-rows-2 gap-y-0 float-left'>
                    ...
                    <del className='text-xs text-gray-500'>{productData?.price * dropItemsNew}</del>
                </ul>
